...
func (t *GenericLoadingTask) MetaDataIdentifier() string {
	...
	} else if t.DBType == db.DynamoDb {
		return strings.Join([]string{t.IdentifierToken, t.ConnStr, t.Extra.Table}, ":")
	} else if t.DBType == db.CouchbaseColumnar {
		return strings.Join([]string{t.IdentifierToken, t.ConnStr, t.Extra.Bucket, t.Extra.Scope, t.Extra.Collection}, ":")
	} else if t.DBType == db.MySql {
		return strings.Join([]string{t.IdentifierToken, t.ConnStr, t.Extra.Database, t.Extra.Table}, ":")
...
func loadDocumentsInBatches(task *GenericLoadingTask) {
	...
	if task.DBType == db.DynamoDb {
		task.Extra.SDKBatchSize = 25
	} else if task.DBType == db.CouchbaseColumnar {
		task.Extra.OperationTimeout = int(task.OperationConfig.End-task.OperationConfig.Start) / 250
	}
	...
	*/
	...
	for i := int64(0); i < numOfBatches; i++ {
		...
		t := newLoadingTask(batchStart+task.OperationConfig.Start,
			...
			task.DatabaseInformation,
			task.Extra,
			external_storage.ExternalStorageExtras{},
			task.req,
			task.MetaDataIdentifier(),
			wg)
		loadBatch(task, t, batchStart, batchEnd)
